...
grid = gridstr.strip().splitlines()
grid = [parseStr(s) for s in grid]
...
def applyOffsets(offsets):
    maxN = 0
    for x in offsets:
        nums = [getNumber(i[0], i[1]) for i in x]
        prod = product(nums)
        if prod > maxN:
            maxN = prod
    return maxN
...
for row in range(0, 20):
    for column in range(0, 20):
        offsets = getOffsets(row, column)
        maxN = applyOffsets(offsets)
        if maxN > maxProduct:
            maxProduct = maxN
